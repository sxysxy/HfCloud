...
void Sprite::calc_rects(){
    if(manager){
        ...
        show_rect = uni;
        clip_rect.x = sprect.x<uni.x?(int)(1.0*(uni.x-sprect.x)*scale_bitmap_width):0;
        clip_rect.y = sprect.y<uni.y?(int)(1.0*(uni.y-sprect.y)*scale_bitmap_height):0;
        clip_rect.w = int(show_rect.w*scale_bitmap_width);
        clip_rect.h = int(show_rect.h*scale_bitmap_height);
    }else{
        clip_rect.x = clip_rect.y = 0;
        clip_rect.w = (int)(_w*scale_bitmap_width), clip_rect.h = (int)(_h*scale_bitmap_height);
